...
func TestKeyIndexTombstone(t *testing.T) {
	...
	err := ki.tombstone(zap.NewExample(), 7, 0, 0)
	if err != nil {
	...
	err = ki.tombstone(zap.NewExample(), 15, 0, 0)
	if err != nil {
		t.Errorf("unexpected tombstone error: %v", err)
	}
	...
	err = ki.tombstone(zap.NewExample(), 16, 0, 0)
...
func newTestKeyIndex() *keyIndex {
	...
	ki.tombstone(zap.NewExample(), 6, 0, 0)
	ki.put(zap.NewExample(), 8, 0, 0)
	ki.put(zap.NewExample(), 10, 0, 0)
	ki.tombstone(zap.NewExample(), 12, 0, 0)
	ki.put(zap.NewExample(), 14, 0, 0)
	ki.put(zap.NewExample(), 14, 1, 0)
	ki.tombstone(zap.NewExample(), 16, 0, 0)
	return ki
}
